...
覆盖层本身，以及内置的应用、文件与计算搜索，只需一个支持上述协议的合成器。其余
功能各自还依赖一个可选组件；缺少时，对应前缀只会返回空结果，而不会报错。更多插件
（Firefox 书签与历史、网页建议等）从
[WayRun-Plugins](https://github.com/Prslc/WayRun-Plugins) 工作区安装：
