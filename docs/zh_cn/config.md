...
| 键 | 默认 | 含义 |
| --- | --- | --- |
| `locale` | *（会话语言）* | 界面语言：`locales/<locale>.yml` 的文件名，如 `zh_cn`，或 `en`。省略时跟随 `$LC_ALL`/`$LC_MESSAGES`/`$LANG`。 |

前端在启动时读取 `locale`（它会把译好的界面文案缓存起来），因此修改后需下次启动生效
（`systemctl --user restart wayrun-launcher`）。没有对应语言表的值会退回英文，留空等同不写这个键。
