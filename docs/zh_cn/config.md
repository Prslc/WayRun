...
| `exclude` | `[]` | 搜索不进入、结果也不显示的目录名。以 `.` 开头的名称（`.git`、`.cache`）一律隐藏，列出也无法找回。随附的 `config.toml` 已列出 `node_modules`、`target`、`__pycache__`：删掉某个名字即可重新搜索它；key 缺失时除隐藏名称外不跳过任何目录。 |

索引在后台建立，且只在你使用 `f` 或 `d` 之后才开始。设 `index = false` 则只搜索 `$HOME`
下 `depth` 层，且不留下缓存；把这两个插件都禁用也是同样效果。修改 `exclude` 后，
索引会在下一次 `f`/`d` 搜索时重建。
